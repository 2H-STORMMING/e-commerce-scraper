      ...
      <div className="w-36 md:w-64">
        {results.map((pageResult) => (
          <div key={pageResult.job_id} className="space-y-2">
            {pageResult.content.results.filters?.map((filter, idx) => (
              <div key={idx} className="border rounded-r-lg md:rounded-lg p-5">
      ...
      <div className="px-5 md:p-10 md:pt-0 space-y-5 flex-1">
        {results.map((pageResult, i) => (
          <div
            ...
            className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid"
          >
            {i !== 0 && <hr className="w-full col-span-full" />}

            <div className="md:col-span-2 lg:col-span-3 xl:span-4 py-5">
            ...
            {pageResult?.content?.results?.organic?.map((item) => (
              ...
              >
                ...
                <div className="px-5 py-2 not-italic">
                  ...
                  <p className="text-[#1B66D2]">{item.merchant.name}</p>
                </div>
              </Link>
            ))}
          </div>
